...
Texture2D gTextureMaps[3] : register(t0);
...
float4 PS(VertexOut pin) : SV_Target
{
    ...
    float TexFactor = 2.2f;
    float3 TexFactor3 = float3(TexFactor, TexFactor, TexFactor);
    float3 albedo =  pow(gTextureMaps[0].Sample(gsamLinearWrap, pin.TexC), TexFactor3); //gDiffuseAlbedo;
    float metallic = gTextureMaps[1].Sample(gsamLinearWrap, pin.TexC).r;
    float roughness = gTextureMaps[2].Sample(gsamLinearWrap, pin.TexC).r;
