// PBR2_Texture.cpp : 定义应用程序的入口点。
//

#include "PBR2_Texture.h"
#include <algorithm>
...
{
	auto albedoTex = std::make_unique<Texture>();
	albedoTex->Name = "albedoTex";
	albedoTex->Filename = L"texture/albedo.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), albedoTex->Filename.c_str(),
		albedoTex->Resource, albedoTex->UploadHeap));

	auto metallicTex = std::make_unique<Texture>();
	metallicTex->Name = "metallicTex";
	metallicTex->Filename = L"texture/metallic.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), metallicTex->Filename.c_str(),
		metallicTex->Resource, metallicTex->UploadHeap));

	auto roughnessTex = std::make_unique<Texture>();
	roughnessTex->Name = "roughnessTex";
	roughnessTex->Filename = L"texture/roughness.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), roughnessTex->Filename.c_str(),
		roughnessTex->Resource, roughnessTex->UploadHeap));

	mTextures[albedoTex->Name] = std::move(albedoTex);
	mTextures[metallicTex->Name] = std::move(metallicTex);
	mTextures[roughnessTex->Name] = std::move(roughnessTex);
}
...
void LitColumnsApp::BuildDescriptorHeaps()
{
	...
	auto albedoTex = mTextures["albedoTex"]->Resource;
	auto metallicTex = mTextures["metallicTex"]->Resource;
	auto roughnessTex = mTextures["roughnessTex"]->Resource;

	D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
	srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
	srvDesc.Format = albedoTex->GetDesc().Format;
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Texture2D.MostDetailedMip = 0;
	srvDesc.Texture2D.MipLevels = albedoTex->GetDesc().MipLevels;
	srvDesc.Texture2D.ResourceMinLODClamp = 0.0f;
	md3dDevice->CreateShaderResourceView(albedoTex.Get(), &srvDesc, hDescriptor);
...
void LitColumnsApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	for (size_t i = 0; i < ritems.size(); ++i)
	{
		...
		CD3DX12_GPU_DESCRIPTOR_HANDLE texDiffuse(mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
		texDiffuse.Offset(ri->Mat->DiffuseSrvHeapIndex, mCbvSrvDescriptorSize);

		cmdList->SetGraphicsRootDescriptorTable(0, texDiffuse);
